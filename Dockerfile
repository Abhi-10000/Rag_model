...
# # Expose the port Cloud Run expects. This is mainly for documentation.
# EXPOSE 8080

# # Start the server using the shell form of CMD.
# # This correctly uses the $PORT variable provided by Google Cloud Run.
# # Make sure your Python file is named 'gang.py' and your FastAPI instance is 'app'.
# CMD uvicorn gang:app --host 0.0.0.0 --port $PORT
FROM python:3.10-slim
...
# Run FastAPI app (change `gang` to the actual main file name if different)
CMD ["uvicorn", "gang:app", "--host", "0.0.0.0", "--port", "8080"]
